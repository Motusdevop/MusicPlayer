...
class MainWindow(QMainWindow):
      ...
      try:
         for track_id in self.playlist.tracks_id:
            if not f"{track_id}.mp3" in os.listdir(self.dir_):
               get_files.get_mp3(track_id, self.dir_)
      except:
         pass

   def add_del_to_playlist(self):
      if self.track.id in self.playlist.tracks_id:
   ...
   def load(self):
      if self.SearchCombox.currentIndex() in (-1, 0):
         pass
      else:
         try:
...
class Alert(QWidget):
   def __init__(self, Alert_text="Alert"):
